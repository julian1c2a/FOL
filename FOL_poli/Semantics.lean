...
namespace FOL_poli.Metamath.Semantics
...
mutual
def evalTerm {D : Type u} (M : Model D) (v : Nat → D) (t : Term) : D :=
  match t with
  | .var n => v n
  | .func f ts => M.func f (evalTerms M v ts)

def evalTerms {D : Type u} (M : Model D) (v : Nat → D) (ts : List Term) : List D :=
  match ts with
  | [] => []
  | t :: ts' => evalTerm M v t :: evalTerms M v ts'
end

def shiftEnv {D : Type u} (v : Nat → D) (d : D) : Nat → D
  | 0 => d
  | n + 1 => v n

def updateEnv {D : Type u} (c : Nat) (v : Nat → D) (d : D) : Nat → D :=
  fun n =>
    if n < c then v n
    else if n = c then d
    else v (n - 1)
...
@[simp]
theorem updateEnv_zero {D : Type u} (v : Nat → D) (d : D) (n : Nat) :
    updateEnv 0 v d n = shiftEnv v d n := by
  cases n <;> simp [updateEnv, shiftEnv]

def evalFormula {D : Type u} (M : Model D) (v : Nat → D) (f : Formula) : Prop :=
  match f with
  | .bottom => False
...
def contextSatisfies {D : Type u} (M : Model D) (v : Nat → D) (Γ : List Formula) : Prop :=
  ∀ f, f ∈ Γ → evalFormula M v f

def satisfies (Γ : List Formula) (f : Formula) : Prop :=
  ∀ (D : Type u) (M : Model D) (v : Nat → D), contextSatisfies M v Γ → evalFormula M v f
...
mutual
theorem eval_liftTerm_ext {D : Type u} (M : Model D) (v : Nat → D) (d : D) (c : Nat) (t : Term) :
...
theorem eval_liftTerms_ext {D : Type u} (M : Model D) (v : Nat → D) (d : D) (c : Nat) (ts : List Term) :
...
mutual
theorem eval_substTerm_ext {D : Type u} (M : Model D) (v : Nat → D) (s : Term) (c : Nat) (t : Term) :
...
theorem eval_substTerms_ext {D : Type u} (M : Model D) (v : Nat → D) (s : Term) (c : Nat) (ts : List Term) :
...
theorem shift_updateEnv_comm {D : Type u} (c : Nat) (v : Nat → D) (d d' : D) :
...
theorem eval_liftFormula_ext {D : Type u} (M : Model D) (v : Nat → D) (d : D) (c : Nat) (f : Formula) :
...
theorem shift_updateEnv_subst_comm {D : Type u} (M : Model D) (c : Nat) (v : Nat → D) (t : Term) (d' : D) :
...
theorem eval_substFormula_ext {D : Type u} (M : Model D) (v : Nat → D) (t : Term) (c : Nat) (f : Formula) :
...
@[simp]
theorem eval_liftFormula_zero {D : Type u} (M : Model D) (v : Nat → D) (d : D) (f : Formula) :
...
@[simp]
theorem eval_substFormula_zero {D : Type u} (M : Model D) (v : Nat → D) (t : Term) (f : Formula) :
...
theorem contextSatisfies_lift_zero {D : Type u} (M : Model D) (v : Nat → D) (d : D) {Γ : List Formula} :
...
-- rule_soundness vale para todo entorno w (no sólo para uno fijo v),
-- lo que permite aplicarla bajo cuantificadores en replaceAt_soundness.
theorem rule_soundness {D : Type u} (M : Model D) {A B : Formula} (h : LocalRule A B) :
    ∀ w : Nat → D, evalFormula M w A ↔ evalFormula M w B := by
  intro w
  cases h with
  | commuteImpl A B C =>
...
-- hEq cuantifica sobre todos los entornos w para poder aplicar la IH
-- bajo cuantificadores (donde el entorno pasa de v a shiftEnv v d).
-- La inducción generaliza también v para que la IH lo parametrice.
theorem replaceAt_soundness {D : Type u} (M : Model D) (v : Nat → D) {f sub sub' : Formula} {p : Pos}
    (hGet : getAt? f p = some sub) (hEq : ∀ w : Nat → D, evalFormula M w sub ↔ evalFormula M w sub') :
  ...
  | left p ih =>
    ...
    | .bottom | .atom _ _ | .eq _ _ | .forall _ | .ex _ =>
  ...
  | body p ih =>
    ...
    | .forall f1 =>
      simp only [getAt?] at hGet
      simp only [replaceAt, evalFormula]
      exact ⟨fun h d => (ih (shiftEnv v d) hGet).mp  (h d),
             fun h d => (ih (shiftEnv v d) hGet).mpr (h d)⟩
    | .ex f1 =>
      simp only [getAt?] at hGet
      simp only [replaceAt, evalFormula]
      exact ⟨fun ⟨d, hd⟩ => ⟨d, (ih (shiftEnv v d) hGet).mp  hd⟩,
             fun ⟨d, hd⟩ => ⟨d, (ih (shiftEnv v d) hGet).mpr hd⟩⟩
    | .bottom | .atom _ _ | .eq _ _ | .impl _ _ | .and _ _ | .or _ _ =>
      simp only [getAt?] at hGet; exact absurd hGet (by simp)

end FOL_poli.Metamath.Semantics
