...
theorem substTerm_lift_comm (t : Term) (c : Nat) (s : Term) :
  ...
  | var n =>
    by_cases h1 : n < c
    · -- n < c: liftTerm c (.var n) = .var n; substTerm leaves it; RHS = .var n
    ...
    · by_cases h2 : n = c
      · -- n = c: liftTerm c (.var c) = .var (c+1); substTerm (c+1) s (.var (c+1)) = s
        -- RHS: substTerm c s (.var c) = s; liftTerm c s
        subst h2
        simp [liftTerm, substTerm]
      · -- n > c: liftTerm c (.var n) = .var (n+1); substTerm (c+1) _ (.var (n+1)) = .var n
        -- RHS: substTerm c s (.var n) = .var (n-1); liftTerm c (.var (n-1))
        -- .var (n-1) with n-1 ≥ c (since n > c) gives .var n
...
theorem substTerm_liftLift (t : Term) (c : Nat) (s : Term) :
  ...
  | var n =>
    by_cases h1 : n < c
    · -- liftTerm c (var n) = var n; substTerm (c+1) s (var n) = var n (n < c+1)
      simp [liftTerm, substTerm, h1, show ¬ n = c + 1 from by omega, show ¬ n > c + 1 from by omega]
    · -- liftTerm c (var n) = var (n+1); liftTerm c (var (n+1)) = var (n+2)
      -- substTerm (c+1) s (var (n+2)): n+2 > c+1 → var (n+1)
