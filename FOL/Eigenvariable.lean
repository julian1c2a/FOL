...
## ⭐ El núcleo es GENÉRICO en un predicado de símbolos (D5, 2026-09-26)

`absTerm' P k` abstrae **toda** constante `func s []` con `P s`. De él salen las dos operaciones
que antes se probaban por separado, con las inducciones hechas **una** vez:

| instancia | es | dónde |
|---|---|---|
| `P := (· = c)` | `absTerm c k` — **por definición** | aquí, §1 |
| `P := fun _ => False` | `liftTerm k` — **por un lema** (`absTerm'_eq_lift`) | `FOL.Lift0` §0 |

⚠️ `liftTerm` NO se redefine como instancia: vive en `FOL/FOL.lean`, raíz del grafo, y tocarlo
recompila todo. El puente es proposicional.
...
| lema | hipótesis |
|---|---|
| `absTerm'_lift` / `absFormula'_lift` | `j ≤ k` |
| `absTerm'_subst` / `absFormula'_subst` | `v ≤ k` |
...
2. `absDerives'` — transporte por los **21** constructores, **generalizado sobre `k`**. ⚠️ El `k`
   ...
   nivel `k+1`, no `k`.
3. `occursFormula` y `absFormula_eq_lift` — si `c` **no aparece**, `absFormula c k` es
...
-- ============================================================
-- §1 · La abstracción de las constantes que cumplen `P`
-- ============================================================

mutual
/-- Sustituye cada **constante** `func s []` con `P s` por `Term.var k`, y levanta las variables
libres `≥ k` para dejarle sitio. -/
def absTerm' {Sym : Type} (P : Sym → Prop) [DecidablePred P] (k : Nat) : TermG Sym → TermG Sym
  | .var n => if n < k then .var n else .var (n + 1)
  | .func s [] => if P s then .var k else .func s []
  | .func s (t :: ts) => .func s (absTerms' P k (t :: ts))

def absTerms' {Sym : Type} (P : Sym → Prop) [DecidablePred P] (k : Nat) :
    List (TermG Sym) → List (TermG Sym)
  | [] => []
  | t :: ts => absTerm' P k t :: absTerms' P k ts
end
...
def absFormula' {Sym : Type} (P : Sym → Prop) [DecidablePred P] (k : Nat) :
    FormulaG Sym → FormulaG Sym
  | .bottom => .bottom
  | .atom p ts => .atom p (absTerms' P k ts)
  | .eq t u => .eq (absTerm' P k t) (absTerm' P k u)
  | .impl a b => .impl (absFormula' P k a) (absFormula' P k b)
  | .forall a => .forall (absFormula' P (k + 1) a)
  | .and a b => .and (absFormula' P k a) (absFormula' P k b)
  | .or a b => .or (absFormula' P k a) (absFormula' P k b)
  | .ex a => .ex (absFormula' P (k + 1) a)

/-- La ecuación de la constante, como `simp`: es la que usa `Henkin0.abs_neg_witness` al
desplegar `absTerm`. -/
@[simp] theorem absTerm'_func_nil {Sym : Type} (P : Sym → Prop) [DecidablePred P] (k : Nat)
    (s : Sym) : absTerm' P k (.func s []) = if P s then .var k else .func s [] := rfl

/-- Abstraer **una** constante `c`. -/
def absTerm {Sym : Type} [DecidableEq Sym] (c : Sym) (k : Nat) (t : TermG Sym) : TermG Sym :=
  absTerm' (fun s => s = c) k t

def absTerms {Sym : Type} [DecidableEq Sym] (c : Sym) (k : Nat) (ts : List (TermG Sym)) :
    List (TermG Sym) :=
  absTerms' (fun s => s = c) k ts

def absFormula {Sym : Type} [DecidableEq Sym] (c : Sym) (k : Nat) (f : FormulaG Sym) :
    FormulaG Sym :=
...
theorem absFormula'_lift (P : String → Prop) [DecidablePred P] :
  ...
  | atom p ts => intro j k h; simp only [absFormula', liftFormula, absTerms'_lift P j k h]
  | eq t u => intro j k h; simp only [absFormula', liftFormula, absTerm'_lift P j k h]
...
theorem absFormula'_subst (P : String → Prop) [DecidablePred P] :
  ...
  | atom p ts => intro v k h s; simp only [absFormula', substFormula, absTerms'_subst P v k h s]
  | eq t u => intro v k h s; simp only [absFormula', substFormula, absTerm'_subst P v k h s]
  | impl a b iha ihb =>
...
/-- **`Derives₀` transporta la abstracción de las constantes que cumplen `P`.**
...
theorem absDerives' (P : String → Prop) [DecidablePred P] {Γ : List Formula} {φ : Formula}
  ...
  | elim_forall Γ' A t _ ih =>
      intro k
      have := Derives₀.elim_forall (Γ'.map (absFormula' P k)) (absFormula' P (k + 1) A)
                (absTerm' P k t) (ih k)
      rw [absFormula'_subst P A 0 k (Nat.zero_le _)]
      exact this
  | intro_ex Γ' A t _ ih =>
      intro k
      refine Derives₀.intro_ex _ _ (absTerm' P k t) ?_
      rw [← absFormula'_subst P A 0 k (Nat.zero_le _)]
      exact ih k
  | elim_ex Γ' A B _ _ ih1 ih2 =>
      intro k
      refine Derives₀.elim_ex _ (absFormula' P (k + 1) A) _ (ih1 k) ?_
      rw [absFormula'_lift P B 0 k (Nat.zero_le _), map_abs'_lift]
      exact ih2 (k + 1)
  ...
  | rewrite_at Γ' f' f'' p sub sub' _ hget hrule heq ih =>
      intro k
      refine Derives₀.rewrite_at _ _ _ p (absFormula' P (k + posDepth p) sub)
        (absFormula' P (k + posDepth p) sub') (ih k) ?_ ?_ ?_
      · rw [abs'_getAt?, hget]; rfl
      · exact abs'_localRule P _ hrule
      · rw [heq, ← abs'_replaceAt]
  ...
  | subst Γ' t₁ t₂ f' _ _ ih1 ih2 =>
      intro k
      have := Derives₀.subst (Γ'.map (absFormula' P k)) (absTerm' P k t₁) (absTerm' P k t₂)
                (absFormula' P (k + 1) f') (ih1 k)
                (by rw [← absFormula'_subst P f' 0 k (Nat.zero_le _)]; exact ih2 k)
      rw [absFormula'_subst P f' 0 k (Nat.zero_le _)]
      exact this
...
theorem absTerm'_eq_lift (P : String → Prop) [DecidablePred P] : ∀ (k : Nat) (t : Term),
  ...
  | func s ts =>
      cases ts with
      | nil =>
          have hs : Not (P s) := hocc s (Or.inl rfl)
          simp [absTerm', liftTerm, liftTerms, hs]
      | cons u us =>
...
theorem absTerms'_eq_lift (P : String → Prop) [DecidablePred P] : ∀ (k : Nat) (ts : List Term),
  ...
  | cons t ts' =>
      simp only [absTerms', liftTerms, List.cons.injEq]
      exact ⟨absTerm'_eq_lift P k t (fun d hd => hocc d (Or.inl hd)),
             absTerms'_eq_lift P k ts' (fun d hd => hocc d (Or.inr hd))⟩
end
...
theorem absFormula'_eq_lift (P : String → Prop) [DecidablePred P] : ∀ (f : Formula) (k : Nat),
  ...
  | atom p ts => intro k h; simp only [absFormula', liftFormula, absTerms'_eq_lift P k ts h]
  | eq t u =>
      intro k h
      simp only [absFormula', liftFormula, absTerm'_eq_lift P k t (fun d x => h d (Or.inl x)),
        absTerm'_eq_lift P k u (fun d x => h d (Or.inr x))]
  | impl a b iha ihb =>
      intro k h
      simp only [absFormula', liftFormula, iha k (fun d x => h d (Or.inl x)),
