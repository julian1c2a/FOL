...
-- Dependencies: FOLPure.FOL, FOLPure.Semantics
-- @axiom_system: classical
-- @importance: high

import FOLPure.FOL
import FOLPure.Semantics
import FOLPure.Deduction
import FOLPure.Theorems.Neg
import FOLPure.Theorems.Quantifiers

namespace FOLPure.Metamath.Completeness
...
theorem DerivesSet_intro_impl {S : Formula → Prop} {A B : Formula}
  ...
  · apply FOLPure.Metamath.Deduction.deduction_theorem
...
theorem max_cons_forall {S : Formula → Prop} (hMax : IsMaximalConsistent S) (hHenkin : IsHenkin S) {A : Formula} :
  ...
  · intro hAll
    ...
    have hExNeg_impl : S ⊢* .impl (neg (.forall A)) (.ex (neg A)) := by
      ...
      · exact FOLPure.Theorems.Quantifiers.forall_not_impl_exists_not
...
theorem completeness {Γ : List Formula} {f : Formula} (h : Γ ⊨ f) : Γ ⊢ f := by
  ...
  have hCons : IsConsistent S := by
    ...
    have hDNE : Γ_sub ⊢ .impl (neg (neg f)) f := FOLPure.Theorems.Neg.dne
...
end FOLPure.Metamath.Completeness

export FOLPure.Metamath.Completeness (
  DerivesSet
  IsConsistent
  IsMaximalConsistent
  IsSatisfiable
