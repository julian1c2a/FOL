...
--
-- Dependencies: FOL_poli.FOL, FOL_poli.Semantics
...
import FOL_poli.Theorems.Eq
universe u

namespace FOL_poli.Metamath.Completeness
...
local notation:50 Γ " ⊨ " f => FOL_poli.Metamath.Semantics.satisfies Γ f
...
theorem DerivesSet_intro_impl {S : Formula → Prop} {A B : Formula}
  ...
  · apply FOL_poli.Metamath.Deduction.deduction_theorem
...
theorem max_cons_forall {S : Formula → Prop} (hMax : IsMaximalConsistent S) (hHenkin : IsHenkin S) {A : Formula} :
  ...
  · intro hAll
    ...
    have hExNeg_impl : S ⊢* .impl (neg (.forall A)) (.ex (neg A)) := by
      ...
      constructor
      · intro g hg; contradiction
      · exact FOL_poli.Theorems.Quantifiers.forall_not_impl_exists_not
...
theorem completeness {Γ : List Formula} {f : Formula} (h : FOL_poli.Metamath.Semantics.satisfies.{0} Γ f) : Γ ⊢ f := by
  apply Classical.byContradiction
  intro hNotDerive
  ...
  have hCons : IsConsistent S := by
    ...
    have hDNE : Γ_sub ⊢ .impl (neg (neg f)) f := FOL_poli.Theorems.Neg.dne
...
end FOL_poli.Metamath.Completeness

export FOL_poli.Metamath.Completeness (
  DerivesSet
  IsConsistent
  IsMaximalConsistent
  IsSatisfiable
